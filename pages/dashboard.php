...
<script>
/*
 * All chart and map initialisation is wrapped in window.addEventListener('load')
 * This guarantees Chart.js (chart.umd.min.js) and Leaflet (leaflet.js) are
 * fully parsed before any new Chart() or L.map() call is made, regardless of
 * where the <script> tags appear in the HTML.
 */
...
// Educational Attainment (horizontal bar)
...
if (typeof L !== 'undefined' && document.getElementById('barangayMap')) {
    ...
    const centerIcon = L.divIcon({
        html: `<div style="width:34px;height:34px;border-radius:50%;background:linear-gradient(135deg,#0d6e5a,#2ecc71);display:flex;align-items:center;justify-content:center;color:#fff;font-size:14px;box-shadow:0 3px 10px rgba(13,110,90,.5),0 0 0 4px rgba(46,204,113,.25);border:2px solid #fff"><i class="fas fa-map-pin"></i></div>`,
        className: '', iconSize: [34,34], iconAnchor: [17,17]
    });
    L.marker([8.3640, 124.9990], { icon: centerIcon })
        .addTo(map)
        .bindPopup(`<div style="font-family:'DM Sans',sans-serif;min-width:160px"><div style="font-weight:700;color:#0d6e5a;font-size:.95rem">📍 Manolo Fortich</div><div style="font-size:.82rem;color:#5a7265;margin-top:.3rem">Barangay SK Headquarters<br>Bukidnon, Philippines</div></div>`, { maxWidth: 220 });
    ...
    function createActivityIcon(status) {
        ...
        return L.divIcon({
            html: `<div style="width:28px;height:28px;border-radius:50%;background:${color};display:flex;align-items:center;justify-content:center;color:#fff;font-size:12px;box-shadow:0 3px 8px ${color}88,0 0 0 3px ${color}33;border:2px solid #fff;cursor:pointer"><i class="fas fa-calendar-check"></i></div>`,
    ...
    ACTIVITY_PINS.forEach(pin => {
        ...
        const sc = STATUS_COLORS[pin.status] || '#9b59b6';
        marker.bindPopup(`
            <div style="font-family:'DM Sans',sans-serif;min-width:220px;max-width:260px">
                <div style="background:${sc};color:#fff;padding:.55rem .85rem;margin:-.75rem -.75rem .75rem;border-radius:8px 8px 0 0;font-weight:700;font-size:.9rem">${pin.title}</div>
                <table style="width:100%;font-size:.8rem;border-collapse:collapse">
                    <tr><td style="color:#9db5a6;padding:.2rem 0;width:80px">Type</td><td style="font-weight:600">${pin.type}</td></tr>
                    <tr><td style="color:#9db5a6;padding:.2rem 0">Status</td><td><span style="background:${sc}22;color:${sc};padding:.15rem .5rem;border-radius:20px;font-weight:700;font-size:.75rem">${pin.status}</span></td></tr>
    ...
    // Expose filter function globally for the select onchange
    window.filterMapMarkers = function(status) {
        allMarkers.forEach(m => {
            if (!status || m._activityStatus === status) m.addTo(map);
